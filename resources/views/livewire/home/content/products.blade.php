<div>
    <h2 class="text-center mt-2">جدیدترین محصولات</h2>
            ...
            <div class="card position-relative" style="background: #F6F6F6">
                ...
                <div class="card-body">
                    <div class="d-flex mb-3 justify-content-between">
                        <span style="font-size:14px ;color:black">{{$product->name}}</span>
                        <span class="text-dark mb-0 text-start" style="font-size:13px; width : 45px">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <p>
                            @if ($product->hasProductQuantity())
                                @if ($product->getProductMinPriceVariation()->sale_price)
                                    <span class="bg-dark text-white p-1">{{number_format($product->getProductMinPriceVariation()->variationDiscount(),1)}}%</span>
                                    <del class="text-muted mx-2" >{{number_format($product->getProductMinPriceVariation()->price)}}</del>
                                    <strong style="color:black ; font-size:16px">{{number_format($product->getProductMinPriceVariation()->sale_price)}} تومان</strong>
                                @else
                                        <strong style="color:black ; font-size:16px">{{number_format($product->getProductMinPriceVariation()->price)}} تومان</strong>
                                @endif
                            @else
